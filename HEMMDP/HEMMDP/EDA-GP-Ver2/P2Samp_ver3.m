function [pos,newbuffer]=P2Samp_ver3(p1,p11,p2,p22,p3,p33,p4,p44,newbuffer,pos)
...
newbuffer(pos)=RAND_START; %dummy assignment,, RAND_START is equavalent to a real number, this assignment is for finding the parent and brother
...
if broth==0
 for k=1:FSET_END
     for u=1:FSET_END
      if (newbuffer(par2)==u && newbuffer(par1)==k)
           p2a=p2(:,k,u);
...
if broth~=0
 for k=1:FSET_END
     for u=1:FSET_END
         for b=1:RAND_END
      if (newbuffer(par2)==u && newbuffer(par1)==k && newbuffer(broth)==b)
           p2b=p22(:,k,u,b);
...
if (0<indx && indx<FSET_END+1)
    [pos,newbuffer]=P3Samp_ver3(p1,p11,p2,p22,p3,p33,p4,p44,newbuffer,pos);  
else
    if broth==0
   [pos,newbuffer]=P2Samp_ver3(p1,p11,p2,p22,p3,p33,p4,p44,newbuffer,pos); %noncomplete tree:[pos,newbuffer]=P2bSamp_ver3(p1,p11,p2,p22,p3,p33,p4,p44,newbuffer,pos);
    elseif uncle1==0
