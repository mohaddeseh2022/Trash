function [p0,p1,p11,p2,p22,p3,p33,p4,p44]=NewP_Final1_New_Formula(buffer,popSize,MI,PTr,par1,par2,par3,par4,par5,broth,n,alpha,BSPlusJ,gen,pstar0,pstar1,pstar11,pstar2,pstar22,pstar3,pstar33,pstar4,pstar44,alph)
...
p0=(alph*pstar0+Ex0)./(sigmakprime0+alph);
p0(isnan(p0))=0;
for k=1:FS_END
Ex1=A1(:,k);  
sigmakprime1(k)=sum(Ex1);
p1(:,k)=(alph*pstar1(:,k)+Ex1)./(sigmakprime1(k)+alph);
p1(isnan(p1))=0;
for b=1:BSPlusJ
Ex11=A11(:,k,b);
sigmakprime11(k,b)=sum(Ex11);
p11(:,k,b)=(alph*pstar11(:,k,b)+Ex11)./(sigmakprime11(k,b)+alph);
p11(isnan(p11))=0;
end
for u=1:FS_END
Ex2= A2(:,k,u);  
sigmakprime2(k,u)=sum(Ex2);
p2(:,k,u)=(alph*pstar2(:,k,u)+Ex2)./(sigmakprime2(k,u)+alph);
p2(isnan(p2))=0;
for b=1:BSPlusJ
Ex22=A22(:,k,u,b);
sigmakprime22(k,u,b)=sum(Ex22);
p22(:,k,u,b)=(alph*pstar22(:,k,u,b)+Ex22)./(sigmakprime22(k,u,b)+alph); 
p22(isnan(p22))=0;
end    
for v=1:FS_END
Ex3=A3(:,k,u,v);
sigmakprime3(k,u,v)=sum(Ex3);
p3(:,k,u,v)=(alph*pstar3(:,k,u,v)+Ex3)./(sigmakprime3(k,u,v)+alph); 
p3(isnan(p3))=0;

for b=1:BSPlusJ 
Ex33=A33(:,k,u,v,b);
sigmakprime33(k,u,v,b)=sum(Ex33);
p33(:,k,u,v,b)=(alph*pstar33(:,k,u,v,b)+Ex33)./(sigmakprime33(k,u,v,b)+alph);
p33(isnan(p33))=0;
end
...
p4(:,k,u,v,y)=(alph*pstar4(:,k,u,v,y)+Ex4)./(sigmakprime4(k,u,v,y)+alph);
p4(isnan(p4))=0;
...
p44(:,k,u,v,y,b)=(alph*pstar44(:,k,u,v,y,b)+Ex44)./(sigmakprime44(k,u,v,y,b)+alph);
p44(isnan(p44))=0;
